...
export async function applyGradient(stops: GradientStop[], strokeWeight: number, startCap: StrokeCap, endCap: StrokeCap, strokeJoin: StrokeJoin): Promise<void> {
    ...
    try {
        ...
        console.log("Old children removed.");
            ...
            if (isFullyOpaque) {
                ...
                getCurrentGradientGroup()!.name = 'Gradient Path';
                ...
                const colorGroup = figma.group(colorSegments, getCurrentGradientGroup()!);
                colorGroup.name = "Color Layer";
                const maskGroup = figma.group(maskSegments, getCurrentGradientGroup()!);
